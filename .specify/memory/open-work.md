...
| OW-013 | 53 historical free-text agent ids are frozen in the agent-id enum baseline; the lint stops NEW drift but retires none of the existing values | residue | bubbles/scripts/agent-id-enum-lint.baseline (landed IMP-036 SCOPE-7). Union measured across the six consuming repos 2026-08-06. REMEDY CORRECTED 2026-08-06 after measuring the actual records: 381 flagged execution records across 139 files, of which 158 are legacy `ralph.*` (agent names that genuinely ran under the old naming - rewriting them falsifies history), 220 are other free-text with NO field naming the real executor (rewriting is guessing), and only 3 are parent-expanded records carrying a usable `expandedBy`. A sampled composite reads `agent: bubbles.implement+test+validate+audit+docs` with `provenanceMode: parent-expanded` and `expandedBy: bubbles.workflow` - ONE run in which the parent did five phases' work because the second hop does not exist | open | control-plane schema owner | Do NOT execute the original remedy of "one entry per agent" for composite ids. Measured against real records it would convert an honest parent-expansion into five specialist dispatches that never occurred - manufacturing exactly the fabrication IMP-036 was written to stop - and it applies to only 3 of 381 records anyway. The load-bearing property is already delivered: the ratchet refuses NEW drift, and doctor now enforces it. Fix aggregation on the CONSUMER side (teach reports to read `provenanceMode`/`expandedBy` and to treat a composite id as one parent-expanded run) rather than by rewriting an audit trail. Let the historical set age out; do NOT add ids to silence a failure, which inverts the ratchet | 2026-08-06 | 2026-08-06 |
