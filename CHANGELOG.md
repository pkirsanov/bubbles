...
### v7.22 reinstated after a brief rollback

- The controlled technical-prose and feature-surface reachability work shipped
  as IMP-030 and IMP-031 was rolled back to the v7.21.0 line on 2026-08-02 and
  is now restored in full, together with its scripts, selftests, registry
  wiring, documentation, skills, and generated assets.
- The rollback also removed a portability fix that was never part of that
  feature work. `security-gate.sh` returned to GNU-only `find -printf`, which
  BSD/macOS `find` rejects, so the gate's non-git branch listed nothing and
  reported OK having inspected nothing. Its selftest fell from 10/10 back to
  5/10. The portable form is restored.
- IMP-032 and IMP-033, and every change that landed after the rollback, are
  retained. BUG-004, BUG-005, and BUG-006 remain open in `BUGS.md`; the rollback
  and its reversal do not resolve or erase those independent defects.
- `bubbles/scripts/docs-registry-effective.sh` is retired again. The rollback
  re-added it as a five-line alias that only re-executes
  `docs-registry-resolve.sh --effective`; v7.22 had retired it as redundant.
  Nothing in the tree references it, so `docs-registry-resolve.sh --effective`
...
- **Source pre-push validation isolates nested Git fixtures.** The maintainer hook clears Git's exported repository-local environment before framework selftests run, preventing fixture `git init` and installer operations from mutating the worktree index being pushed. A poisoned-environment selftest verifies both variable removal and nested-repository resolution.
...
- Git hook environment sanitization: 3/3 assertions.
