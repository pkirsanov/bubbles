...
- **Repo-binding preflight (`repo-binding-preflight.sh`)** — a fail-loud guard
  (IMP-025 / BFW-05) against the multi-root hazard where a session edits one
  repository while running a Bubbles agent installed under a DIFFERENT workspace
  root (Feature-010 edited Research Lab using GuestHost's / QuantitativeFinance's
  agents). It refuses (exit 1) before mutable work when the agent's source-repo
  slug does not match the repository being edited, naming both repos + the
  remediation; passes for `--canonical-source` framework work; and is advisory
  (exit 0) only when no binding marker is available (never blocks what it cannot
  verify). Reuse-first: it uses the SAME repo-slug derivation the installer
  already computes for the per-repo MCP server id (`install.sh` `mcp_repo_slug`)
  and reads a forward-compatible `targetRepoSlug` from
  `.github/bubbles/.install-source.json` — no per-machine absolute path. Hermetic
  `repo-binding-preflight-selftest.sh` (matching pass, foreign refuse naming both,
  canonical pass, marker auto-detect match/foreign, no-marker advisory, slug
  sanitization) wired into `framework-validate.sh`. Installer/agent marker
  stamping + repo-qualified agent labels + handoff-envelope provenance are the
  documented follow-up. VERSION not bumped.
