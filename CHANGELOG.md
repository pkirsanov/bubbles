...
- **Risk-tier resolver (`risk-tier-resolve.sh`)** — a mechanical, FAIL-CLOSED
  eligibility resolver for the risk-tiered rapid-delivery path (IMP-021 / BFW-01,
  SCOPE-1). It resolves `rapid-tool-delivery` ONLY when a change shows a positive
  low-risk signal (build-free/static/single-file/isolated tool) AND carries NONE
  of the high-risk triggers (auth, payments, secrets, PII, DB migration,
  deployment/infra, production mutation, host-singleton, cross-product contract),
  checked against both the request surface and the changed-path families. Any
  trigger — or the absence of a positive low-risk signal — resolves to
  `full-delivery`, so a user can NEVER self-label risky work low-risk to shed
  gates and unknown/ambiguous work gets maximum assurance. Reuse-first: it adds
  NO budget mechanism (Gate G128 already enforces aggregate session budgets) and
  registers NO mode — the rapid path prefers defaulting an existing focused mode
  - setting G128 budgets + this router (the mode/routing wiring is the follow-up).
  Hermetic `risk-tier-resolve-selftest.sh` (low-risk pass; each high-risk trigger
  class escalates; low-risk-signal-plus-trigger still escalates; path-based
  trigger; fail-closed ambiguous) wired into `framework-validate.sh`. VERSION not
  bumped.
