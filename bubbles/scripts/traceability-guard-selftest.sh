...
: "${BUG039_REAL_PYTHON:?missing real python path}"
...
status=0
  ...
  if [[ "${1:-}" == "-" ]]; then
    ...
    "$BUG039_REAL_PYTHON" "$script_capture" "$@" >"$stdout_capture" 2>"$stderr_capture" || status=$?
  else
    "$BUG039_REAL_PYTHON" "$@" >"$stdout_capture" 2>"$stderr_capture" || status=$?
  fi
cat "$stdout_capture"
...
SHIM
  ...
  CASE_STATUS=0
  if PATH="$shim_dir:$PATH" BUG039_REAL_PYTHON="$real_python" \
    bash "$GUARD" "$feature_dir" "$@" >"$case_log" 2>&1; then
    CASE_STATUS=0
  else
    CASE_STATUS=$?
