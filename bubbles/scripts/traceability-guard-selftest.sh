...
run_trace_case() {
  ...
  if [[ "$feature_dir" == "$TMPDIR/"* ]]; then
    feature_arg="${feature_dir#"$TMPDIR"/}"
    run_root="$TMPDIR"
  fi
  ...
  CASE_STATUS=0
  if [[ -n "$scope_mode" ]] && (cd "$run_root" && bash "$GUARD" "$feature_arg" "$scope_mode") >"$case_log" 2>&1; then
    CASE_STATUS=0
  elif [[ -z "$scope_mode" ]] && (cd "$run_root" && bash "$GUARD" "$feature_arg") >"$case_log" 2>&1; then
    CASE_STATUS=0
  else
    CASE_STATUS=$?
  fi
...
run_trace_case_system_bash() {
  ...
  case_log="$TMPDIR/bug018-case-${CASE_INDEX}.log"
  CASE_STATUS=0
  if (cd "$run_root" && env -i HOME="$HOME" PATH=/usr/bin:/bin:/usr/sbin:/sbin \
...
SHIM
  ...
  case_log="$TMPDIR/bug018-case-${CASE_INDEX}.log"
  CASE_STATUS=0
  if (cd "$run_root" && PATH="$shim_dir:$PATH" BUG018_REAL_AWK="$real_awk" \
...
run_b046_trace_case_system_bash() {
  ...
  case_log="$TMPDIR/b046-case-${CASE_INDEX}.log"
  CASE_STATUS=0
  if (cd "$repo_dir" && env -i HOME="$HOME" PATH=/usr/bin:/bin:/usr/sbin:/sbin \
...
set +e
(cd "$TMPDIR" && bash "$GUARD" "${clean_feature#"$TMPDIR"/}") >"$log1" 2>&1
status1=$?
set -e
...
set +e
(cd "$TMPDIR" && bash "$GUARD" "${broken_feature#"$TMPDIR"/}") >"$log2" 2>&1
status2=$?
set -e
...
set +e
(cd "$TMPDIR" && bash "$GUARD" "${declared_feature#"$TMPDIR"/}") >"$log3" 2>&1
status3=$?
set -e
...
set +e
(cd "$TMPDIR" && bash "$GUARD" "${ambiguous_feature#"$TMPDIR"/}") >"$log4" 2>&1
status4=$?
set -e
