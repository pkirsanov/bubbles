...
run_bug050_regression_case() {
  local bug050_receipt_repo="$tmp_root/bug050-receipt-repo"
  local bug050_receipt_feature="$bug050_receipt_repo/specs/950-bug050-transition-local"
  local bug050_receipt_log="$bug050_receipt_repo/.specify/runtime/tool-calls.jsonl"
  local bug050_active_input_hash
  local bug050_active_output_hash
  local bug050_log_hash_before
  local bug050_log_rows_before
  local bug050_guard_log
  local bug050_guard_status
  local bug050_log_hash_after
  local bug050_log_rows_after
  ...
  python3 - "$bug050_receipt_feature/state.json" <<'PY'
import json
import sys
...
with open(state_file, encoding="utf-8") as handle:
    state = json.load(handle)
state["execution"]["completedPhaseClaims"] = ["docs"]
...
if [[ "${BUBBLES_STATE_TRANSITION_GUARD_BUG050_ONLY:-0}" == "1" ]]; then
  mkdir -p "$tmp_root/specs" "$tmp_root/.specify/memory"
  clone_framework_surface "$tmp_root"
  git -C "$tmp_root" init -q
  export BUBBLES_REPO_ROOT="$tmp_root"
  ...
  echo "----------------------------------------"
  if [[ "$failures" -gt 0 ]]; then
    echo "state-transition-guard BUG-050 selftest failed with $failures issue(s)."
    exit 1
  fi
  echo "state-transition-guard BUG-050 selftest passed."
  exit 0
fi
