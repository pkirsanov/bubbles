...
def load_registry() -> dict[str, Any]:
    ...
    exact(value, {"adapters","bridges","classifications","contractType","coverageStates","evidenceRelations","materialityVerdicts","operations","recordTypes","schemaVersion","selectorTypes","stageRegistry","telemetryFields","version"}, "runtime registry")
...
class Runtime:
    ...
    def execute_stage(self, stage: str, question: dict[str, Any], plan: dict[str, Any], req: dict[str, Any], mbe_context: dict[str, Any], outputs: dict[str, list[dict[str, Any]]], inputs: list[str]) -> list[dict[str, Any]]:
        ...
        if stage == "optional-synthesis":
            ...
            candidate = local_command(req["adapter"]["configPath"], candidate_input(question, outputs))
...
def validate_artifact(question: dict[str,Any], artifact: dict[str,Any], outputs: dict[str,list[dict[str,Any]]]) -> dict[str,Any]:
    ...
    checks={"identity":not findings,"citations":coverage["state"]=="satisfied","coverage":coverage["state"],"conflicts":assessment["verdict"] not in {"human","refuse"},"privacy":True,"freshness":"stale" not in {row["state"] for row in coverage["dimensions"]},"budget":True}
...
def local_command(config_path: str, input_value: dict[str,Any]) -> dict[str,Any]:
    ...
    if cancel is not None and Path(cancel).exists(): fail("RER-ROUTE-UNAVAILABLE","cancelled","dispatch cancelled before start")
    ...
    try: stdout,stderr=process.communicate(stdin,timeout=timeout_ms/1000)
    except subprocess.TimeoutExpired:
        os.killpg(process.pid,signal.SIGTERM)
        try: process.wait(timeout=1)
        except subprocess.TimeoutExpired: os.killpg(process.pid,signal.SIGKILL); process.wait()
        fail("RER-ROUTE-UNAVAILABLE","timeout","local command exceeded deadline")
    if len(stdout)>max_output or len(stderr)>max_error: fail("RER-SOURCE-LIMIT","limit","adapter output exceeded configured bound")
