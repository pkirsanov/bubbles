...
def validate_host_proof(value: Any) -> dict[str, Any]:
    proof = exact_fields(value, {"contractType", "schemaVersion", "proofType", "proofId", "issuer", "previousSessionIdentity", "nextSessionIdentity", "continuationDigest", "verificationDigest", "verified"}, "hostProof")
    if proof["contractType"] != "independently-verifiable-host-proof" or proof["schemaVersion"] != 1:
        malformed("host proof contract is unsupported")
    ...
    digest(proof["verificationDigest"], "hostProof.verificationDigest")
    if proof["verified"] is not True:
        refused("MBE-EPOCH-BOUNDARY-UNVERIFIED", "host proof is not independently verified")
...
class MeasuredBudgetRuntime:
    ...
    def budget_settle(self, *, budget_id: str, reservation_id: str, permit_id: str, consumption_id: str, usage_receipt_id: str, receipt_verification_id: str, revision: int, supersedes_settlement_id: str | None, partitions: list[dict[str, Any]], terminal_state: str, settled_at: str) -> dict[str, Any]:
        ...
        with self.store.locked():
            ...
            event_amounts = [{"dimension": row["dimension"], "amount": row[terminal_state], "unit": row["unit"], "currency": row["currency"], "scale": row["scale"]} for row in normalized_partitions]
            event_type = {"debit": "DEBIT", "release": "RELEASE", "hold": "HOLD"}[terminal_state]
            event = self._budget_event(policy, event_type, reservation_id, settlement["settlementId"], reservation["occurrenceId"], reservation["attemptId"], reservation["actionDigest"], event_amounts, usage_receipt_id, None, settled_at, "verified-receipt-settlement")
            self._append_locked(event, "budget", event["budgetEventId"], reservation["occurrenceId"], reservation["attemptId"])
            return settlement
    ...
    def epoch_boundary(self, *, goal_id: str, from_epoch_id: str, to_epoch_class: str, boundary_kind: str, previous_session_identity_id: str, next_session_identity_id: str, continuation_digest: str, budget_snapshot_id: str, host_proof_digest: str, observed_at: str) -> dict[str, Any]:
        ...
        with self.store.locked():
            records = self._records_locked()
            next_identity = self._find(records, "host-session-identity", "sessionIdentityId", next_session_identity_id)
            ...
            if next_identity["proofDigest"] != host_proof_digest:
                refused("MBE-EPOCH-BOUNDARY-UNVERIFIED", "independent host identity proof digest mismatch")
    ...
    def epoch_verify(self, *, boundary_id: str, host_proof_digest: str, verified_at: str) -> dict[str, Any]:
        ...
        with self.store.locked():
            ...
            verdict = "verified" if boundary["hostProofDigest"] == host_proof_digest else "invalid"
            payload = {"boundaryId": boundary_id, "goalId": boundary["goalId"], "previousSessionIdentityId": boundary["previousSessionIdentityId"], "nextSessionIdentityId": boundary["nextSessionIdentityId"], "continuationDigest": boundary["continuationDigest"], "hostProofDigest": host_proof_digest, "verdict": verdict, "verifiedAt": verified_at}
