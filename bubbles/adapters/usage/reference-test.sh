...
case "$OPERATION" in
  ...
  verify-receipt)
    ...
    verification_payload="$(jq -cS '{proofDigest:.receipt.sourceProofDigest,revision:1,supersedesVerificationId:null,usageReceiptId:.receipt.usageReceiptId,verdict:"valid",verifiedAt:.verifiedAt}' "$INPUT_FILE")"
