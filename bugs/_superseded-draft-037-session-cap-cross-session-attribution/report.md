...
but none names or specifies G128 cross-session attribution. BUG-037 is the next
root packet identifier after BUG-036.
...
The planning artifacts now describe all eight `SCN-B037-*` contracts, 20
...
| --- | --- | --- |
| `BUG037-IMP-001` repository-wide G128 attribution | Fixed in the candidate | Receipts 20 and 25 |
| `BUG037-IMP-002` malformed cap and record types could collapse into default-off or empty collections | Fixed after a current-session RED | Receipts 10 and 25 |
| `BUG037-IMP-003` stock Bash 3 treated the empty goal-node argument array as unbound | Fixed after a current-session RED | Receipts 22 through 25 |
| `BUG037-IMP-004` generated release metadata was stale | Regenerated only with the canonical generator | Receipt 27 |
| `BUG037-IMP-005` exact-session regressions lacked several planned adversarial assertions | Strengthened without weakening planned behavior | Receipts 25 and 32 |
...
This independent observation is `OBS-B037-001` in the result envelope.
...
| --- | --- | --- |
| `RF-B037-01` | Addressed. Packet validation now precedes target path derivation. Descriptor locks use no-follow opens and preserve planted bytes. | Rows 98 and 103 |
| `RF-B037-03` | Addressed. Exact-session policies use append-only compare-and-append revisions. Legacy and unrelated state remain preserved. | Rows 98 and 99 |
| `BUG037-S1-PARTIAL-001` | Addressed. A first policy write no longer requires a pre-existing memory directory. | RED row 86 and GREEN row 98 |
| `BUG037-S1-PARTIAL-002` | Addressed for future writes. Persistent flock state moved under the ignored runtime tree. | RED row 82 and GREEN rows 98 and 103 |
| `BUG037-S1-PARTIAL-003` | Addressed. Duplicate policy options now fail before packet or state access. | RED row 82 and GREEN row 98 |
| `BUG037-S1-PARTIAL-004` | Addressed. Every orchestrator contract now preserves all seven values and explicit nulls. | RED row 83 and GREEN row 99 |
| `BUG037-S1-HARDEN-001` | Addressed. Holder reads now verify one stable regular entry. Private and holder writes require complete descriptor writes. | Row 98 |
...
| --- | --- | --- | ---: | --- |
| TP-01-01 | `SCN-B037-010` | Tool log row 134, stdout SHA-256 `ef9578e164664c71358672885e971a73ae727a6a5ae7564a1d4dc8b0857f5644` | 14 files | 117 assertions passed across 21 cases |
| TP-01-02 | `SCN-B037-009` | Tool log row 135, stdout SHA-256 `30cfe6a41ac536bd772c41c24ef7f7c86c1aae519f108fa776ca2a2005cbbdc5` | 14 files | 117 assertions passed across 21 cases |
| Compatibility | `SCN-B037-010` | Tool log row 137, stdout SHA-256 `aa2374acd8df60c23e0af5ffea8bcd75ec22dfd1685167e66c5817a1231e88f3` | 14 files | Homebrew Bash repeated all 117 assertions |
...
| Test row | Scenario binding | Receipt | Input closure | Direct result |
| --- | --- | --- | ---: | --- |
| TP-01-03 | `SCN-B037-011` | Tool log row 136, stdout SHA-256 `561ac7497516e4fc7aa81a4c1db26d6ef06181fbc8c7b888936015b9007dd045` | 16 files | Exact-session autonomy selftest passed |
| Compatibility | `SCN-B037-011` | Tool log row 138, stdout SHA-256 `7f1c0b162a138c9a147c7074c771a65c52da409f88ea739a5b77f23b62050827` | 16 files | Homebrew Bash repeated the same verdict |
...
| Finding | Disposition | Evidence |
| --- | --- | --- |
| `RF-B037-04` | Addressed. G128 captures one descriptor-safe state revision. It selects one linear exact-session policy head and validates every matching event. | Rows 153 through 155 |
| `RF-B037-06` | Addressed. The shared helper emits JSON strings. All untrusted guard values use that encoder before output. | Rows 149 and 155 |
| `BUG037-S2-001` | Addressed. Malformed known caps now retain a safe dimension diagnostic before identity evaluation. | Final selftest row 155 |
| `BUG037-S2-EVIDENCE-001` | Addressed. Two helper checks invoked the non-executable helper path incorrectly. Rows 149 and 150 replaced those invalid checks through explicit Python. | Rows 147 through 150 |
...
| --- | --- | ---: | --- | --- |
| 143 | `SCN-B037-001` and `TP-02-01` | 1 | `7e0ce3825820bc73a15f28d14d3ca6d8bf62ad3c020fe6d3dc2d68d81c79ecb8` | Live-path rereads and missing immutable revision assertions failed. |
| 144 | `SCN-B037-002` and `TP-02-02` | 1 | `9b5109c4f9619208e9ba15b6827f75d766ec83598ad21644e9b1b855d4206674` | Closed boundary status assertions failed. |
| 145 | `SCN-B037-006` and `TP-02-03` | 1 | `b7d09811e89c84d5357154716ffa5ffec69727993681770f038f5c0668a4aa70` | Policy-chain, escaping, and safe-state assertions failed. |
...
Tool-log row 153 binds `SCN-B037-001` to the production guard and helper. The
receipt carries eight current-byte inputs and the immutable-state negative
control.
...
Tool-log row 154 binds `SCN-B037-002` to the production G128 guard. The
negative control moves one observation from equality to one unit above.
...
Tool-log row 155 binds `SCN-B037-006` to exact policy and escaping controls.
...
Tool-log row 175 binds only `SCN-B037-001`. Its eight-file input closure
...
Tool-log row 176 binds only `SCN-B037-002`. Its eight-file input closure
...
Tool-log row 177 binds only `SCN-B037-006`. Its eight-file input closure
...
| --- | ---: | --- |
| `TEST-B037-S2-CMD-001` | Row 163 exited 1 because inline Python quoting was malformed. Its broad `xit(` pattern also matched `last_exit()`. | Row 164 used in-memory Python syntax and a token-boundary skip scan. |
| `TEST-B037-S2-CMD-002` | Row 166 exited 1 before mutation execution because one inline f-string escaped quotes incorrectly. | Row 167 executed all five disposable mutations. |
| `TEST-B037-S2-PRES-001` | Row 168 exited 0 but parsed backslash-zero instead of NUL. Its one-path manifest is inadmissible. | Row 169 parsed `bytes([0])`, required 32 protected paths, and preserved every identity. |
| `TEST-B037-S2-CERT-001` | Row 170 used a newline-bearing `jq` rendering against a newline-free Python baseline hash. The values were not comparable. | Row 172 used the original canonical Python method and matched the pre-edit certification hash. |
| `TEST-B037-S2-PROSE-001` | Row 171 saw zero prose findings but its regex expected a literal backslash sequence. | Row 172 parsed rule prefixes and integer tails without regex escaping. |
| `TEST-B037-S2-CLOSURE-001` | Rows 159 through 161 passed before scenario evidence references were added. Their declared manifest input then became stale. | Rows 175 through 177 repeated all three TP rows against the final scenario-manifest bytes. |
...
Tool-log row 183 records the linked-test resolver. It resolved all 59 BUG-037
scenario references and exited 0. Its captured output has SHA-256
...
The output contains every BUG-037 exact-selector assertion. None used the
...
| 203 | BUG-037 artifact lint | 0 | `182cf27f7948b167f9fdebccae5bf6994636355face5d8ae0a4d55666dc9b567` |
...
Scope 3 is independently verified. BUG-037 remains `in_progress`.
...
writer for the BUG-037 artifacts.
...
| Finding | Disposition | Evidence |
| --- | --- | --- |
| `RF-B037-02` | Addressed. Check 40 and framework validation validate actionable packet authority, require a stable available guard, and accept only the closed child status and exit matrix. | Rows 230 and 238 |
| `RF-B037-07` | Addressed. G082 filters the authoritative host session and target spec while retaining the per-spec maximum. | Row 231 |
...
| 255 | Focused BUG-037 artifact lint | 0 | All required artifacts and anti-fabrication checks passed; complete output SHA-256 `182cf27f7948b167f9fdebccae5bf6994636355face5d8ae0a4d55666dc9b567` |
...
current bytes. BUG-037 remains `in_progress`. Certification and human
