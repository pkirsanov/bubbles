# BUG-037 Design - Authoritative Host-Session Cap Enforcement
...
None. The specification and RF-B037-01 through RF-B037-09 resolve the required
behavior.

## Purpose And Scope

BUG-037 repairs the existing post-activity G082 and G128 enforcement path. It
...
| --- | --- | --- |
| RF-B037-01 | Validate actionable authority before repository-local path derivation. Use safe flock and mkdir lock contracts. | Invalid authority creates no repository entry. Planted targets remain byte-identical. |
| RF-B037-02 | Give Check 40 and framework validation a validated packet context. Require one closed child status and exit pair. | Ambient-only identity, unavailable guards, and malformed child results block. |
| RF-B037-03 | Add exact-session append-only budget records with one active chain head. Preserve independent all-null policy. | Two sessions retain different values and independent default-off behavior. |
| RF-B037-04 | Capture one immutable session revision. Validate timestamps, budget keys, and present byte members completely. | A replacement affects only a later invocation. Invalid matching input never yields a subset total. |
| RF-B037-05 | Traverse usage roots completely without following links. Select and parse one stable exact object. | Traversal, transport, containment, read, stability, or parse failure returns `INPUT-ERROR`. |
| RF-B037-06 | JSON-quote every user-controlled diagnostic value. Keep trusted tokens closed. | Newline, tab, escape, quote, and delimiter inputs produce one physical diagnostic line. |
| RF-B037-07 | Add exact-session G082 filtering while retaining its target-spec maximum. | Another session cannot change G082. G128 still sums the active session across specs. |
| RF-B037-08 | Require persistent, concurrency, caller, lock, mapping, and interpreter proof that matches executed paths. | Each claimed path executes with row-level and interpreter assertions. |
| RF-B037-09 | Freeze managed inputs before generation and validation. | Manifest generation, freshness check, framework validation, and release check share one unchanged epoch. |
...
`reason=no-exact-producer` under BUG-037.
...
| --- | --- | --- |
| RF-B037-01 | `state-snapshot-selftest.sh`, safe-I/O helper selftest | Invalid authority leaves zero repository entries. Flock and mkdir targets reject symlinks, non-regular files, and replacement races without byte changes. |
| RF-B037-02 | `state-transition-guard-selftest.sh`, `framework-validate-tier-selftest.sh` | Full packet validation precedes invocation. Every status-count, vocabulary, exit-pair, and guard-availability case uses the closed result. |
| RF-B037-03 | `state-snapshot-selftest.sh`, `session-cap-guard-selftest.sh`, `autonomy-resolve-selftest.sh` | Two sessions preserve different policy chains. One bounded session does not activate an all-null or absent sibling. |
| RF-B037-04 | `session-cap-guard-selftest.sh`, safe-I/O helper selftest | One state revision backs a verdict. Invalid matching timestamps, unknown budget keys, and present invalid byte members fail closed. |
| RF-B037-05 | `usage-adapter-contract-selftest.sh` | Newline filenames, symlinks, traversal failure, unreadable files, replacement, mixed tokens, and malformed JSON cannot produce neutral or partial measured data. |
| RF-B037-06 | direct-guard and caller selftests | Control-byte arguments remain one escaped line and cannot add a final status. |
| RF-B037-07 | `convergence-cap-guard-selftest.sh`, `state-transition-guard-selftest.sh` | G082 excludes another session and retains the target-spec maximum. G128 retains the session-wide sum. |
| RF-B037-08 | runtime concurrency, tool-log, persistent regression, platform matrix | Exact row deltas, every mapping, both lock verdict pairs, callers, G082, and actual child interpreters are proven. |
| RF-B037-09 | manifest check, framework validation, release check | All final commands run after generation on one unchanged candidate. |
...
- `maxToolCalls` is always `UNMEASURABLE` in BUG-037.
...
BUG-037 repairs attribution in the existing post-activity G128 reader. It does
...
BUG-037 creates no scalar replacement, sample array, counter writer, or
...
| `maxToolCalls` | No observation exists in BUG-037. | `no-exact-producer` |
...
| --- | --- | --- | --- |
| SCN-B037-001 | functional | `bubbles/scripts/session-cap-guard-selftest.sh` | Old over-cap turns and convergence do not charge the active ID. |
| SCN-B037-002 | functional | `bubbles/scripts/session-cap-guard-selftest.sh` | Equality passes and one unit above fails for each measurable dimension. |
| SCN-B037-003 | functional | `bubbles/scripts/session-cap-guard-selftest.sh` | Old oversized bytes are excluded and the active twin breaches. |
| SCN-B037-004 | functional | `bubbles/scripts/session-cap-guard-selftest.sh` | Old cumulative bytes are excluded and an active added row breaches. |
| SCN-B037-005 | integration | `bubbles/scripts/runtime-concurrency-selftest.sh` and `bubbles/scripts/state-snapshot-selftest.sh` | Same-spec, same-agent sessions retain two keys and independent totals. |
| SCN-B037-006 | functional | `bubbles/scripts/session-cap-guard-selftest.sh` | Legacy rows and scalar survive, remain excluded, and tool calls stay unmeasurable. |
| SCN-B037-007 | integration | `bubbles/scripts/usage-adapter-contract-selftest.sh` | Exact unique identity measures. Zero, prefix, ambiguous, and unscoped identity abstain. |
| SCN-B037-008 | regression | `tests/regression/test_22_session_cap_enforcement.sh` | Cap names, nulls, equality, soft boundary, history, and bypass refusal remain intact. |
...
BUG-037 adds no admission check, permit, reservation, session epoch, goal
