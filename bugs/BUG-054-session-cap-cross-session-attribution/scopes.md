...
| --- | ---: | --- |
| RF-B054-01 | 1 | Authority precedes repository side effects. Flock and mkdir targets are no-follow and identity checked. |
| RF-B054-02 | 4 | Check 40 and framework validation require actionable authority, installed guards, and a closed child result. |
| RF-B054-03 | 1 | Exact-session policy chains preserve different values and independent default-off posture. |
| RF-B054-04 | 2 | One immutable revision backs complete policy, timestamp, and byte-shape validation. |
| RF-B054-05 | 3 | Exact usage discovery is complete, contained, no-follow, stable, and lossless. |
| RF-B054-06 | 2 | Shared JSON-string escaping keeps each untrusted value on one physical line. |
| RF-B054-07 | 4 | G082 filters the authoritative session while retaining its target-spec maximum. |
| RF-B054-08 | 5 | Persistent and concurrent proof checks rows, mappings, both locks, callers, G082, and child interpreters. |
| RF-B054-09 | 5 | Release metadata is generated after final managed edits and checked on an unchanged candidate. |
...
| --- | ---: | --- |
| FR-B054-001 | 2 | TP-02-03 |
| FR-B054-002 | 2 | TP-02-01 |
| FR-B054-003 | 1 | TP-01-02 |
| FR-B054-004 | 2 | TP-02-03 |
| FR-B054-005 | 3 | TP-03-01, TP-03-03 |
| FR-B054-006 | 3 | TP-03-02, TP-03-03 |
| FR-B054-007 | 2 | TP-02-01 |
| FR-B054-008 | 2 | TP-02-03 |
| FR-B054-009 | 2 | TP-02-02 |
| FR-B054-010 | 1 | TP-01-02 |
| FR-B054-011 | 4 | TP-04-02, TP-04-03 |
| FR-B054-012 | 5 | TP-05-01, TP-05-04, TP-05-05 |
| FR-B054-013 | 5 | TP-05-06, TP-05-07, TP-05-08 |
| FR-B054-014 | 2 | TP-02-03 |
| FR-B054-015 | 2 | TP-02-03 |
| FR-B054-016 | 5 | TP-05-04, TP-05-06 |
| FR-B054-017 | 4 | TP-04-02, TP-04-03 |
| FR-B054-018 | 1 | TP-01-02, TP-01-03 |
| FR-B054-019 | 3 | TP-03-02, TP-03-03 |
| FR-B054-020 | 3 | TP-03-02 |
| FR-B054-021 | 4 | TP-04-02, TP-04-03 |
| FR-B054-022 | 4 | TP-04-02, TP-04-03 |
| FR-B054-023 | 2 | TP-02-01 |
| FR-B054-024 | 1 | TP-01-01, TP-01-02 |
| FR-B054-025 | 2 | TP-02-01, TP-02-03 |
| FR-B054-026 | 2 | TP-02-03 |
| FR-B054-027 | 2 | TP-02-03 |
| FR-B054-028 | 4 | TP-04-01, TP-04-02 |
| FR-B054-029 | 3 | TP-03-01, TP-03-03 |
| FR-B054-030 | 5 | TP-05-03, TP-05-04, TP-05-05 |
| FR-B054-031 | 5 | TP-05-01, TP-05-04, TP-05-05 |
| FR-B054-032 | 5 | TP-05-01, TP-05-02, TP-05-03 |
| FR-B054-033 | 5 | TP-05-04, TP-05-05 |
...
- Plan-owned routing and `workBoundary` fields in `bugs/BUG-054-session-cap-cross-session-attribution/state.json`
...
**Root Findings:** RF-B054-01, RF-B054-03
...
```gherkin
Scenario: SCN-B054-009 Authority fails before repository side effects
...
Scenario: SCN-B054-010 Safe lock targets cannot redirect or corrupt state
...
Scenario: SCN-B054-011 Exact-session policy histories remain independent
...
| --- | --- | --- | --- | --- | --- | --- |
| TP-01-01 | SCN-B054-010 | functional | `bubbles/scripts/state-snapshot-selftest.sh` | Helper-level descriptor cases reject symlink, non-regular, replacement, and stale-identity lock targets without changing sentinel bytes. | `/bin/bash bubbles/scripts/state-snapshot-selftest.sh` | Yes, real filesystem and locks |
| TP-01-02 | SCN-B054-009 | integration | `bubbles/scripts/state-snapshot-selftest.sh` | Packet rejection precedes repository entries. Valid writes preserve exact policy chains, turns, mirrors, and convergence mappings under flock and mkdir locks. | `/bin/bash bubbles/scripts/state-snapshot-selftest.sh` | Yes, real producer, binding validator, filesystem, and locks |
| TP-01-03 | SCN-B054-011 | functional | `bubbles/scripts/autonomy-resolve-selftest.sh` | Unattended boundedness reads only the exact session head. A bounded sibling or legacy object cannot activate an absent or all-null session. | `/bin/bash bubbles/scripts/autonomy-resolve-selftest.sh` | Yes, production resolver over ephemeral state |
...
- [x] Test TP-01-01 passes inside the state snapshot selftest for SCN-B054-010 safe descriptor and lock refusal. → Evidence: [record](report.md#scope-1-final-state-and-lock-evidence)
- [x] Test TP-01-02 passes for SCN-B054-009 through SCN-B054-011 state integration. → Evidence: [record](report.md#scope-1-final-state-and-lock-evidence)
- [x] Test TP-01-03 passes for SCN-B054-011 exact policy selection and default-off isolation. → Evidence: [record](report.md#scope-1-final-autonomy-evidence)
...
**Root Findings:** RF-B054-04, RF-B054-06
...
```gherkin
Scenario: SCN-B054-001 Old-session event history is excluded
...
Scenario: SCN-B054-002 Exact measured boundaries retain both directions
...
Scenario: SCN-B054-006 Legacy data and invalid active input never become usage
...
| ID | Scenario | Type | File | Required assertion | Command | Live System |
| --- | --- | --- | --- | --- | --- | --- |
| TP-02-01 | SCN-B054-001 | functional | `bubbles/scripts/session-cap-guard-selftest.sh` | The real guard uses one immutable revision and rejects an incomplete matching event population instead of measuring a subset. | `/bin/bash bubbles/scripts/session-cap-guard-selftest.sh` | Yes, production guard over ephemeral state |
| TP-02-02 | SCN-B054-002 | functional | `bubbles/scripts/session-cap-guard-selftest.sh` | Exact observations preserve equality, one-unit-over breach, strict greater-than, and measured-only 70 percent boundary behavior. | `/bin/bash bubbles/scripts/session-cap-guard-selftest.sh` | Yes, production guard over ephemeral state |
| TP-02-03 | SCN-B054-006 | functional | `bubbles/scripts/session-cap-guard-selftest.sh` | Invalid identity, policy keys, revisions, timestamps, and arguments yield escaped INPUT-ERROR while default-off and all seven dimension states remain truthful. | `/bin/bash bubbles/scripts/session-cap-guard-selftest.sh` | Yes, production guard over ephemeral state |
...
- [x] Test TP-02-01 passes for SCN-B054-001 immutable event isolation and complete-population validation. → Evidence: [TP-02-01 immutable event evidence](report.md#scope-2-tp-02-01-immutable-event-evidence)
- [x] Test TP-02-02 passes for SCN-B054-002 equality, strict breach, and soft-boundary calculations. → Evidence: [TP-02-02 exact boundary evidence](report.md#scope-2-tp-02-02-exact-boundary-evidence)
- [x] Test TP-02-03 passes for SCN-B054-006 identity, schema, diagnostics, and default-off behavior. → Evidence: [TP-02-03 policy and diagnostic evidence](report.md#scope-2-tp-02-03-policy-and-diagnostic-evidence)
...
```gherkin
Scenario: SCN-B054-003 Single-result bytes are session isolated
...
Scenario: SCN-B054-004 Cumulative bytes are session isolated
...
Scenario: SCN-B054-007 Prompt-token accounting requires one stable exact artifact
...
- [x] Test TP-03-01 passes for SCN-B054-003 and SCN-B054-004 receipt production. → Evidence: [Scope 3 TP-03-01 receipt evidence](report.md#scope-3-tp-03-01-receipt-evidence)
- [x] Test TP-03-02 passes for SCN-B054-007 exact usage discovery and failure semantics. → Evidence: [Scope 3 TP-03-02 exact usage evidence](report.md#scope-3-tp-03-02-exact-usage-evidence)
- [x] Test TP-03-03 passes for SCN-B054-003, SCN-B054-004, and SCN-B054-007 G128 consumption. → Evidence: [Scope 3 TP-03-03 receipt consumption evidence](report.md#scope-3-tp-03-03-receipt-consumption-evidence)
...
**Root Findings:** RF-B054-02, RF-B054-07
...
```gherkin
Scenario: SCN-B054-012 G082 and G128 share authority but retain distinct aggregation
...
Scenario: SCN-B054-013 Blocking callers accept only one closed child result
...
Scenario: SCN-B054-014 Blocking authority and guard availability fail closed
...
- [ ] Test TP-04-01 passes for SCN-B054-012 G082 isolation and aggregation.
- [ ] Test TP-04-02 passes for SCN-B054-012 through SCN-B054-014 transition callers.
- [ ] Test TP-04-03 passes for SCN-B054-013 and SCN-B054-014 framework validation.
...
**Root Findings:** RF-B054-08, RF-B054-09
...
```gherkin
Scenario: SCN-B054-005 Concurrent sessions preserve exact rows, mappings, and verdicts
...
Scenario: SCN-B054-008 Existing policy and platform contracts remain intact
...
Scenario: SCN-B054-015 Release metadata belongs to the final unchanged candidate
...
| ID | Scenario | Type | File | Required assertion | Command | Live System |
| --- | --- | --- | --- | --- | --- | --- |
| TP-05-01 | SCN-B054-005 | integration | `bubbles/scripts/runtime-concurrency-selftest.sh` | Real concurrent producers prove exact receipt deltas, every session mapping, both policy heads, and both G082 and G128 verdict pairs after flock and mkdir locking. | `/bin/bash bubbles/scripts/runtime-concurrency-selftest.sh` | Yes, concurrent production processes and filesystem |
| TP-05-02 | SCN-B054-005 | integration | `bubbles/scripts/tool-log-selftest.sh` | Concurrent completed wrapper calls add exactly one physical row per expected session. Duplicate rows cannot collapse behind a keyed projection. | `/bin/bash bubbles/scripts/tool-log-selftest.sh` | Yes, concurrent real wrapper calls |
| TP-05-03 | SCN-B054-008 | integration | `bubbles/scripts/state-snapshot-selftest.sh` | Final state tests prove both lock paths, exact convergence and policy mappings, invalid-authority zero-side-effect behavior, and actual child interpreter records. | `/bin/bash bubbles/scripts/state-snapshot-selftest.sh` | Yes, real producer, validator, locks, and state |
| TP-05-04 | SCN-B054-008 | functional | `tests/regression/test_22_session_cap_enforcement.sh` | Stock macOS Bash 3 executes every claimed production path with adversarial controls and verifies each changed production child's actual interpreter identity. | `/bin/bash tests/regression/test_22_session_cap_enforcement.sh` | Yes, persistent production-path regression |
| TP-05-05 | SCN-B054-008 | functional | `tests/regression/test_22_session_cap_enforcement.sh` | The supported Bash and GNU environment preserves the same statuses, exits, mappings, diagnostics, and negative controls. | `bash tests/regression/test_22_session_cap_enforcement.sh` | Yes, persistent cross-platform regression |
| TP-05-06 | SCN-B054-015 | functional | `bubbles/scripts/generate-release-manifest.sh` | The generated manifest is fresh for every final managed source, test, contract, and planning byte. | `bash bubbles/scripts/generate-release-manifest.sh --check` | No, deterministic generated-artifact check |
| TP-05-07 | SCN-B054-015 | functional | `bubbles/scripts/cli.sh` | Full framework validation executes the focused and persistent BUG-054 paths on the frozen candidate. | `bash bubbles/scripts/cli.sh framework-validate` | Yes, complete framework validation surface |
| TP-05-08 | SCN-B054-015 | functional | `bubbles/scripts/cli.sh` | Release readiness reruns full validation and derived-artifact freshness on the same unchanged candidate. | `bash bubbles/scripts/cli.sh release-check` | Yes, source release gate |
...
- [ ] SCN-B054-008 preserves every cap value, null, strict comparison, 70 percent soft boundary, retained history, no-bypass behavior, and G085 behavior.
...
- [ ] Test TP-05-01 passes for SCN-B054-005 row-level concurrency and both lock verdict pairs.
- [ ] Test TP-05-02 passes for SCN-B054-005 exact concurrent receipt multiplicity.
- [ ] Test TP-05-03 passes for final state, lock, mapping, authority, and child-interpreter integration.
- [ ] Test TP-05-04 passes under stock macOS Bash 3 for persistent SCN-B054-001 through SCN-B054-015 coverage.
- [ ] Test TP-05-05 passes under the supported Bash and GNU environment for the same persistent scenarios.
- [ ] Test TP-05-06 passes after final generation and proves SCN-B054-015 metadata freshness.
...
PLAN-B054-001 is addressed by this reconciled five-scope sequence. All nine
RF-B054 roots map once to an owning scope. Execution begins with Scope 1 only.
