# BUG-054 Expected Behavior - Host-Session-Scoped G128 Measurement
...
BUG-054 keeps G128 as a post-activity session-wide gate and G082 as a
...
| --- | --- | --- | --- |
| RF-B054-01 | F08 | Validate packet authority before any repository-local directory, lock, or state side effect. Reject unsafe lock targets without following or truncating them. | FR-B054-024 |
| RF-B054-02 | F01, F05, F06 | Bind Check 40 and framework validation to actionable packet authority. Enforce one closed child status and exit pair. Missing enforcement fails closed. | FR-B054-017, FR-B054-021, FR-B054-022 |
| RF-B054-03 | F02 | Select and preserve one budget for the exact host session. Concurrent sessions retain independent values and default-off posture. | FR-B054-018 |
| RF-B054-04 | F07, F09, F10, F13 | Evaluate one immutable session revision. Reject incomplete timestamps, unknown budget keys, and invalid present byte fields. | FR-B054-023, FR-B054-025, FR-B054-026, FR-B054-029 |
| RF-B054-05 | F03, F04 | Make exact usage discovery complete, contained, no-follow, stable, and lossless for valid path bytes. Traversal, read, or parse failure cannot become neutral or partial data. | FR-B054-019, FR-B054-020 |
| RF-B054-06 | F11 | Escape every rejected argument as one stable diagnostic value. Control bytes cannot alter line or status framing. | FR-B054-027 |
| RF-B054-07 | F12 | Bind G082 to the same active session authority while retaining its per-spec maximum. The active work boundary must admit its required source and tests. | FR-B054-028 |
| RF-B054-08 | F14, F15, F16, F17, F19 | Require truthful persistent and concurrency proof for exact mappings, both lock verdict pairs, packet-authority callers, G082, and interpreter identity. | FR-B054-030 through FR-B054-033 |
| RF-B054-09 | New independent freshness finding | Regenerate and recheck release metadata after the final managed source, test, contract, and expected-behavior changes. | FR-B054-013 |
...
tenth root. F19 remains an evidence-integrity input under RF-B054-08.
...
| --- | --- | --- |
| F01 | In scope — FR-B054-017 | Check 40 and live framework validation forward ambient `BUBBLES_SESSION_ID` without validating it against actionable packet authority. |
| F02 | In scope — FR-B054-018 | G128 reads one top-level `sessionBudget`, and the goal-agent seeding contract names that shared location without a session key. |
| F03 | In scope — FR-B054-019 | The VS Code adapter converts exact-file read failures to neutral output and selects only objects carrying `promptTokens`, which permits malformed or mixed populations to disappear. |
| F04 | In scope — FR-B054-020 | Adapter enumeration suppresses traversal errors, transports paths by newline, counts pathnames, and later reopens the selected pathname without containment or object-stability proof. |
| F05 | In scope — FR-B054-021 | The live framework wrapper prints a pass for every child exit 0, even when the parsed status is empty or outside the closed success set. |
| F06 | In scope — FR-B054-022 | Check 40 reports a missing or non-executable blocking G128 guard as an advisory skip. |
| F07 | In scope — FR-B054-023 | G128 validates and rereads the shared session pathname through many separate `jq` processes while the producer can atomically replace that pathname. |
| F08 | In scope — FR-B054-024 | `state-snapshot.sh` derives the packet-controlled root and opens its repository-local lock before `mirror-session` validates the packet. The flock open can follow a planted symlink. |
| F09 | In scope — FR-B054-025 | The timestamp projection catches invalid matching timestamps as empty and can measure the remaining subset. |
| F10 | In scope — FR-B054-026 | G128 validates seven known cap values but never rejects additional `sessionBudget` keys. |
| F11 | In scope — FR-B054-027 | Unknown flags and positional arguments are interpolated raw into diagnostics. Control and newline bytes can therefore change output framing. |
| F12 | In scope — FR-B054-028 | The changed producer adds session-specific convergence rows, but G082 still selects only by spec path and takes the maximum across every retained session. |
| F13 | In scope — FR-B054-029 | Matching byte validation accepts present `null` values and later coerces them to zero, although the tool-call schema permits only integers when those properties are present. |
| F14 | In scope — FR-B054-030 | Several Bash 3 entry tests invoke changed production children through PATH-resolved `bash`. The outer interpreter alone does not prove the child's interpreter. |
| F15 | In scope — FR-B054-031 | The persistent test invokes the direct guard. It does not execute snapshot production, Check 40, live validation, same-key concurrency, or G082 while claiming all eight scenarios. |
| F16 | In scope — FR-B054-032 | The concurrent tool-log assertion builds a dictionary keyed by session ID. Duplicate rows collapse before cardinality is checked. |
| F17 | In scope — FR-B054-032 | Same-key concurrency checks counts and unique IDs, not every session-to-iteration pair. Independent verdicts run only after the primary lock path. |
| F18 | Disproved as a BUG-054 product finding | Receipt row 48 passed the bug directory to `scenario-compile-lint.sh`. That command requires a compiled scenario JSON file, so exit 1 records an invalid test invocation rather than a product defect. The receipt is retained and excluded from BUG-054 evidence. |
| F19 | In scope evidence correction — FR-B054-033 | Receipt row 50 is tagged `RED` but records exit 0 and no scenario binding. The receipt remains immutable and inadmissible as RED evidence. |

F01 through F17 and F19 map exactly once into RF-B054-01 through RF-B054-08.
F18 remains disproved. RF-B054-09 adds the current release-freshness defect.
...
| `maxToolCalls` | None with current production sources. | Always unmeasurable in BUG-054. The unattributed scalar and tool-log row count are ineligible. |
...
G082 and G128 are command-line control-plane surfaces. BUG-054 adds no web page,
...
A plan that forbids a required source cannot claim coverage of RF-B054-07.

## Exposure Contract

| Capability | Surface class | Surface ID | Status | Plan |
| --- | --- | --- | --- | --- |
| Exact-session G128 evaluation | cliCommand | `bash bubbles/scripts/session-cap-guard.sh --session-id <id> [--quiet]` | planned | BUG-054 |
| Exact-session G082 evaluation | cliCommand | `bash bubbles/scripts/convergence-cap-guard.sh <specDir> --session-id <id> [--quiet]` | planned | BUG-054 |
| Session-bound budget selection | internal | validated host-session budget projection | planned | BUG-054 |
| Transition enforcement | internal | Check 40 in `tail-convergence-gates.sh` | planned | BUG-054 |
| Live framework validation | internal | G128 live check in `framework-validate.sh` | planned | BUG-054 |
| Exact prompt-token projection | internal | usage adapter `session <id>` | planned | BUG-054 |
| Safe session-state persistence | internal | validated state-snapshot producer | planned | BUG-054 |
| Final release metadata freshness | internal | release-manifest freshness check | planned | BUG-054 |

## Use Cases

### UC-B054-001 - Evaluate One Active Session
...
### UC-B054-002 - Reject An Unattributable Evaluation
...
### UC-B054-003 - Evaluate Concurrent Sessions Independently
...
### UC-B054-004 - Reject Unsafe Or Partial Measurement Input
...
### UC-B054-005 - Enforce Through A Blocking Caller
...
### UC-B054-006 - Preserve Safe Concurrent State
...
- **FR-B054-001:** An active-budget G128 evaluation must consume exactly one
  non-empty `--session-id` value supplied by the host-aware caller.
- **FR-B054-002:** G128 must filter turn snapshots by exact `hostSessionId`.
- **FR-B054-003:** The convergence producer must record `hostSessionId` and key
  updates by session, spec, and agent.
- **FR-B054-004:** `maxToolCalls` must remain unmeasurable because current
...
- **FR-B054-005:** G128 must filter tool-log byte rows by exact `sessionId`.
- **FR-B054-006:** G128 must request prompt-token usage for the exact active
  session. Zero, prefix-only, multiple, and unscoped matches must abstain.
- **FR-B054-007:** Unattributed and mismatched records must never affect active
  totals, boundary percentages, or breach decisions.
- **FR-B054-008:** G128 must report every configured dimension as measured or
  unmeasurable. Exit 0 must not imply that every dimension was measured.
- **FR-B054-009:** All seven cap names, values, null semantics, and greater-than
...
- **FR-B054-010:** Turn snapshots and tool receipts must remain append-preserved.
  Convergence must preserve latest-value semantics within the expanded exact
  session, spec, and agent key.
- **FR-B054-011:** Check 40 and live framework validation must forward the same
  exact host-supplied ID. They must preserve exit 1 versus exit 2 diagnostics.
- **FR-B054-012:** Focused and persistent regressions must execute the real
  guard and real producer paths.
- **FR-B054-013:** Generated release metadata must be refreshed after the final
...
- **FR-B054-014:** When any session-bound policy has a non-null cap, missing,
...
- **FR-B054-015:** Diagnostics must identify the active ID, dimension states,
  observations or reasons, and excluded record counts without host-private
  paths or inferred ownership.
- **FR-B054-016:** BUG-054 must not introduce any IMP-055 admission or session
  epoch capability, a new tool-call producer, a bypass, or a source `specs/`
  directory.
- **FR-B054-017:** Every blocking G082 or G128 caller must validate one
...
- **FR-B054-018:** Budget selection must be exact-session scoped. Concurrent
...
- **FR-B054-019:** Once one exact usage artifact is selected, malformed JSON or
...
- **FR-B054-020:** Exact usage enumeration must propagate traversal and read
...
- **FR-B054-021:** Check 40 and live framework validation must enforce a closed
...
- **FR-B054-022:** A missing, non-regular, or non-executable blocking G128 guard
  must fail Check 40 and live framework validation. It must never become an
  advisory skip or pass.
- **FR-B054-023:** G128 must capture one immutable, safely opened session-state
...
- **FR-B054-024:** `state-snapshot.sh` must validate its private binding packet
  ...
  target.
- **FR-B054-025:** Every matching turn that participates in wall-clock
  ...
  excluded.
- **FR-B054-026:** The session-budget schema must accept only the seven existing
...
- **FR-B054-027:** Every user-controlled value in diagnostics must use stable
...
- **FR-B054-028:** G082 must consume the same validated active-session identity
...
- **FR-B054-029:** For a matching byte-bearing receipt, an absent
...
- **FR-B054-030:** Stock macOS Bash 3 portability evidence must prove the
...
- **FR-B054-031:** Persistent regression claims must match the production paths
...
- **FR-B054-032:** Concurrent producer tests must assert the exact append delta
...
- **FR-B054-033:** Evidence classification must follow the executed command and
...
BUG-054 has no rendered interface. These wireframes define the terminal
...
| --- | --- | --- | --- |
| Direct G128 result | Host-session-aware caller | Existing - Modify | SCN-B054-001 through SCN-B054-008 |
| Direct G082 result | Host-session-aware caller | Existing - Modify | SCN-B054-001, SCN-B054-005, SCN-B054-008 |
| Check 40 result | Transition caller | Existing - Modify | SCN-B054-001, SCN-B054-002, SCN-B054-006, SCN-B054-008 |
| Framework validation result | Validation caller | Existing - Modify | SCN-B054-001, SCN-B054-002, SCN-B054-006, SCN-B054-007, SCN-B054-008 |
...
6. `maxToolCalls` always reports `UNMEASURABLE` with
  `reason=no-exact-producer` in BUG-054.
