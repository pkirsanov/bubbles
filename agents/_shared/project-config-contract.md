...
3. `.github/agents/_shared/agent-common.md` — Universal agent governance
4. `.github/agents/_shared/scope-workflow.md` — Universal workflow governance
...
- [ ] `.github/agents/_shared/` — Copy all shared files as-is (project-agnostic)
- [ ] `.github/scripts/bubbles-*.sh` — Copy governance scripts as-is (project-agnostic)

**The Bubbles agent files (`.github/agents/`) and shared governance (`.github/agents/_shared/`) MUST NOT be modified per-project.** They are universal. Only the three configuration files listed above are project-specific.
...
| `.github/agents/_shared/agent-common.md` | Universal governance: anti-fabrication (G019-G030), evidence standards, test taxonomy, quality work standards | Uses `[cmd]` placeholders and indirection references |
| `.github/agents/_shared/critical-requirements.md` | Top-priority non-negotiable policy set (no fabrication, no stubs/TODOs/fallbacks/defaults, full implementation and validation) | Project-agnostic governance, no project-specific values |
| `.github/agents/_shared/scope-workflow.md` | Universal workflow: DoD templates, phase exit gates, artifact templates, status ceiling, state.json canonical schema | Uses `[cmd]` placeholders |
| `.github/agents/_shared/project-config-contract.md` | This file — the cross-project interface contract | Describes the interface, not the implementation |
| `.github/agents/_shared/feature-templates.md` | Feature artifact templates | Structure-only, no project references |
| `.github/agents/_shared/bug-templates.md` | Bug artifact templates | Structure-only, no project references |
| `.github/agents/_shared/docker-lifecycle-governance.md` | Docker lifecycle governance (freshness, cleanup, labeling) | Universal Docker patterns, no project references |
...
# N. Run artifact lint
bash .github/scripts/bubbles-artifact-lint.sh specs/<NNN-feature-name>
```
...
| Gate | Requirement |
|------|-------------|
| ✅ **State Transition Guard (Gate G023)** | `bash .github/scripts/bubbles-state-transition-guard.sh specs/<NNN>` exits 0. |
...
```markdown
> **Authoritative source:** [`agent-common.md` → Anti-Fabrication Policy](agents/_shared/agent-common.md)
```
...
ls -la .github/scripts/bubbles-state-transition-guard.sh

# 2. Artifact lint exists and is executable
ls -la .github/scripts/bubbles-artifact-lint.sh
...
bash .github/scripts/bubbles-state-transition-guard.sh specs/<any-spec>
```
