...
- Traits owing LIVE proof must have it: Playwright or equivalent on the current production route for user-visible UI, a real request and response for an API contract, a write-and-read round trip for mutable state, a live boundary for freshness, retry, queue, provider or dependency behavior, and stress or load for SLA claims. A synthetic test may complement an applicable live proof; it may never replace one.
