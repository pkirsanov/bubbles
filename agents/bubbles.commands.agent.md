...
Search for design/architecture docs in common locations (e.g., `DESIGN.md`, `docs/ARCHITECTURE.md`, `docs/DESIGN.md`, `design/README.md`, `architecture/README.md`, `README.md`). Also inventory standard docs from `docs/` (API, DEVELOPMENT, TESTING, DEPLOYMENT, TROUBLESHOOTING, PRACTICES) and `specs/`/`.specify/` when present. If user provides `--design=<path>`, use that path.
