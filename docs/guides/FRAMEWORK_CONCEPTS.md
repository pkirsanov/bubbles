...
| **State transition guard** | Runs 41 checks before any status transition to "done" |
...
Prose governance documents are necessary for context but insufficient for enforcement. AI agents routinely ignore or reinterpret written rules. The breakthrough came from **scripts that mechanically block state transitions** when conditions aren't met. The state transition guard alone runs 41 automated checks. When an agent can't advance without passing a script, compliance stops being optional.
