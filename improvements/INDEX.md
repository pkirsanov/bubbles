...
| IMP-036 | Delivery efficiency: close the dispatch hole, instrument the gates, and cut the ceremony that buys nothing | ACCEPTED 2026-08-06 — SCOPE-5's original "delete the propagation instruction" wording was falsified by its own pre-deletion check and corrected in place: the capability is fully wired (guard, gates G121-G123, schema, template) and degrades correctly to exit 0 when no policy file exists, so the fix is to drop the `applyTo: "**"` header, not the file. Generalised into a reusable test — policy conditional on a configuration surface belongs in a skill, only unconditional policy belongs always-on. SCOPE-1 remains gated on confirming the second-hop diagnosis against a traced dispatch | framework-health (G125) | HO-1, HO-2, EV-6, COV-6, COST-2, COST-3, REG-6, REG-7 | 2026-08-06 |
