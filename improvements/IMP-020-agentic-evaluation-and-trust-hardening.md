...
**Status:** IN PROGRESS — S1, S2, and S3 implemented and validated; S4-S7 pending. Repository owner approval recorded 2026-07-10. (S3 delivered 2026-07-18 in commit `53085ce`: `agents/bubbles_shared/untrusted-content.md` data-vs-instruction policy + module-index registration; `bubbles/tool-trust-registry.yaml` + Draft-07 `tool-trust-registry.schema.json` validated by `yaml-schema-validate.sh`; `pre-tool-risk-gate.sh --event` fail-closed structured decision (unregistered/unknown deny, action-bound host-verified approval binding tool|server|op|target|dataClasses|egress with replay/expiry/unverified rejection, `--confirm`/env can never unlock sensitive, `hostEnforceable=false` ⇒ `enforcement:unavailable`+block, secret never egresses); +17 `pre-tool-risk-gate-selftest.sh` cases (27 pass, auto-run by `framework-validate`); `docs/recipes/tool-trust-and-untrusted-content.md` + `docs/MCP.md`. All S3 acceptance criteria met; the pre-tool-risk-gate selftest is green.) (S2 delivered under the v7.20.0 convergence commit `cb13053`: `adversarial-resolve.sh` canonical `samples` + deprecated `passes` alias + `sampleSemantics=same-runtime-correlated`; `adversarial-aggregate.sh` deterministic union/agreement/disagreement + selftest wired into `framework-validate` as "Adversarial aggregate selftest (IMP-020 / S2)"; `adversarial-sample.schema.json`; honest terminology in `bubbles.redteam`/`bubbles.super`/`agent-common.md`; `docs/recipes/cross-model-review.md` reframed as "Unavailable Capability And Migration Note". All five S2 acceptance criteria verified met 2026-07-17.)
