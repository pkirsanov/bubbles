...
**Progress 2026-07-19 (R4 terminal-state foundation, framework-validate green):** `transition-contract-resolver.sh` now accepts a mode's declared `terminalAliases[]` as legal terminal current-statuses (previously it only accepted the nominal `statusCeiling`, silently rejecting any alias — a latent gap). This is the low-risk, plan-sanctioned foundation for `delivered_fast`: a mode may now honestly terminate at an alias (e.g. `delivered_fast`) instead of `done`, consistent with `is-terminal-for-mode.sh`, WITHOUT touching the deliberately-locked `transitionAudit.profile` schema enum (kept `["delivery-completion-v1","planning-maturity-v1"]`). Resolver selftest +3 cases (declared alias accepted; alias contract carries the alias current status over the `done` ceiling; undeclared alias still rejected) → 59/0. Remaining R4: (R4b) give `rapid-tool-delivery` `terminalAliases: [delivered_fast]` + teach the completion consumers (spec-dashboard, release-train WARN lists) — carefully, since deploy-eligibility of `delivered_fast`/`delivered_prototype` is coupled to Phase 3; (R4c) the validate-owned `assurance{…}` metadata block; execution substates + IMP-024 receipts.
