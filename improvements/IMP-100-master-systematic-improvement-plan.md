...
## 4. Phase 1 — Risk-proportional delivery (the churn fix) · absorbs IMP-021 + IMP-003 (budgets)
...
**DISCOVERED FINDING (2026-07-18, full framework-validate): `eval-harness-selftest.sh` fails 1/104 on macOS** — the "invalid printf judge output" case sets `BUBBLES_EVAL_JUDGE=/usr/bin/printf` and expects error code `judge-malformed-json`, but macOS `/usr/bin/printf` emits output that `eval-harness.sh` (classify chain: `empty-output` → `malformed-json` → `schema-invalid`) buckets as a different code. Pre-existing (files clean at HEAD; not this session's guard/skill work), platform-portability gap. Fix under this phase: make the printf-judge fixture platform-deterministic or classify a macOS hollow `/usr/bin/printf` judge output as `malformed-json`. Until fixed, a full framework-validate is RED on this one check only.
