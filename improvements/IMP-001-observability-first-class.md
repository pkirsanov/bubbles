...
> **Status:** FRAMEWORK SHIPPED — v7.10.0 delivered SCOPE-1..6 to `origin/main` (commit `1baa796`, 2026-06-11); v7.10.1 follow-up hardens downstream enforcement + consumer proof. SCOPE-7 (downstream dogfood) CLOSED 2026-06-13 via QuantitativeFinance spec `085-observability-slo-dogfood` (status `done`): a wired downstream repo now carries a real `slo:` link backed by captured `.specify/runtime/observability/*.slo.json` telemetry that G100 enforces green. SCOPE-9 / T9.4 PROGRESSED 2026-06-14 — a **second** wired repo, **smackerel** spec `090-observability-slo-dogfood` (status `done`, pushed `d52df324`), now carries a real `core.health` `slo:` link backed by captured live `/api/health` telemetry (p99 26–36ms, 0% err, 100% avail) that G100 enforces green, proving the pattern generalises beyond the first repo. Remaining T9.x wired repos (wanderaide, guesthost) await clean per-repo windows; the dogfood is the propagation template.
...
- [x] A downstream repo posture declared and doctor output recorded. **Claim Source: executed (2026-06-12).** posture=wired; posture-guard rc=0; pushed `<sha>`.
- [x] A downstream repo posture declared and doctor output recorded. **Claim Source: executed (2026-06-12).** posture=wired; posture-guard rc=0; pushed `<sha>`.
- [x] Wired repos have one workflow + SLO + validate-plane evidence path; opted-out repos have complete decision metadata. **Claim Source: executed (2026-06-14).** opted-out (knb) decision metadata COMPLETE (reasonCode/reason/revisitAfter present, opt-out-guard rc=0). Wired repos — T9.4 instrumentation now PROVEN on a **second** wired repo: **smackerel** spec `090-observability-slo-dogfood` (status `done`, pushed `d52df324`) carries a real `traceContracts.observability.slos.core.health` + `workflows.core.health.slo` link backed by captured `.specify/runtime/observability/core.health.slo.json` telemetry (live `/api/health` on :40001, 600@20 + 1500@50 stress, p99 organically 26–36ms across runs, 0% err, 100% avail) that G100 enforces GREEN (no-op → ENFORCED, exit 0). Second wired repo after QF spec `085` (SCOPE-7); the box's `≥1 wired repo carries a real slo: link` threshold is met. Remaining T9.x wired repos (wanderaide, guesthost) await their own clean per-repo windows — both were skipped this cycle under an operational-safety hold (each had a hot concurrent session), and guesthost additionally has a pre-existing dev-backend config-generator gap (the dev `docker-compose.yml` `environment:` block omits `STRIPE_REQUIRES_ACTION_TIMEOUT_MIN`, so its `/health` bring-up fail-closes).
