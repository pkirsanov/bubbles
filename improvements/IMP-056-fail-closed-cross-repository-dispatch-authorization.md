...
### SCOPE-2 — Broker Action/Executable Snapshot And Crash-State Hardening — **DONE except one bullet**
...
- **STILL OPEN, in progress.** `launch-pending`/`launch-confirmed`/`launch-denied`/`launch-ambiguous` events and the recovery path — this is the one real SCOPE-2 gap; work is underway (`measured-budget-runtime.py` `launch_pending`/`launch_confirm`/`launch_deny`/`launch_reconcile`, new `dispatch-launch-state` contract type registered in `measured-budget-runtime-contracts.json`), not yet wired into `reference-broker.sh` or covered by a selftest.
- **STILL OPEN**, pending the same work: preserve one-use permit semantics so that on unresolved post-consumption state, the full reservation is held and automatic retry is prohibited.
- ~~Add adversarial tests that mutate action files, executable paths, shebang interpreters, and explicit interpreters after validation...~~ **DONE, and already landed before this write-up was corrected.** `measured-budget-runtime-v2-selftest.py`'s `TypedGraphTests` carries this coverage; an earlier version of this document incorrectly claimed it did not exist — corrected in the Status line above.
