...
**Status:** IN PROGRESS — SCOPE-1 (planning) landed prior to 2026-09-05. SCOPE-2 (broker snapshot/executable hardening) fully landed 2026-09-05: its one real gap, the launch-state ledger (`launch-pending`/`launch-confirmed`/`launch-denied`/`launch-ambiguous`), is now implemented in `measured-budget-runtime.py`, wired into `reference-broker.sh`, and covered by 12 new tests (10 pure-runtime, macOS-verified; 2 broker-level, Linux-dependent like their siblings), 2 guards mutation-verified. SCOPE-3 (signed mutable-dispatch authorization) landed 2026-09-05: `mutable-dispatch-authorization.sh` mint/verify, reusing `security-authority.py` under purpose `mutable-dispatch-authorization`; 17/17 selftest, mutation-verified, all 11 named negative-coverage cases present. Investigation ahead of SCOPE-3 found ONE real, open gap in SCOPE-2 — and, on closer reading, ONE claim about SCOPE-2 that was wrong and is corrected here rather than left standing: the `launch-pending`/`launch-confirmed`/`launch-denied`/`launch-ambiguous` ledger vocabulary SCOPE-2 itself specifies genuinely does not exist anywhere in the tree (confirmed by a literal repo-wide string search). The claim that SCOPE-2's required adversarial broker selftest was ALSO missing was **false** — it exists and is substantial: `measured-budget-runtime-v2-selftest.py`'s `TypedGraphTests` class carries real, subprocess-level adversarial coverage against `reference-broker.sh` (action-file substitution, executable-path replacement, symlink/shebang/explicit-interpreter refusal, group-writable-executable refusal, malformed-consumption refusal), and it IS wired into `framework-validate.sh` transitively — the registered `measured-budget-runtime-selftest.py` dynamically imports and runs it. The broker itself is Linux-only by design (`reference-broker-snapshot.py` refuses any non-Linux platform before its capability probe), so these tests cannot be exercised on a macOS development sandbox; running the suite there with `jsonschema` installed shows exactly the broker-dependent tests erroring on the Linux-only precondition and every non-broker test passing, which is the expected shape, not a defect. SCOPE-4 (canonical final-edge gateway) landed 2026-09-05: new `mutable-dispatch-gateway.sh` composes adapter resolution, one shared broker-owned snapshot, and mint+verify of the authorization before ever invoking the broker (via a new, additive `--existing-snapshot-dir` flag on `reference-broker.sh` that avoids re-deriving the snapshot and reopening the TOCTOU window that would create); 5/6 selftest cases pass on macOS, the sixth needs the same Linux-only precondition every SCOPE-2 broker-launch test needs; 2 guards mutation-verified; SCOPE-2's full 44-test suite re-run with zero regressions. SCOPE-6 still owns removing the direct-broker-call bypasses this gateway does not itself close. SCOPE-5 landed 2026-09-05: `reference-broker.sh`'s own `reference-dispatch-result` gained a conditional `mutableDispatchAudit` field (permit, authorization, launch-state identifiers), present only for gateway-originated dispatches, sourced entirely from durable records after the fact; confirmed by repo-wide search to have zero consumers anywhere, so it cannot feed back into any authorization decision.
...
### SCOPE-2 — Broker Action/Executable Snapshot And Crash-State Hardening — **DONE**

- ~~Add a capability probe and closed support matrix for immutable launch strategies.~~ **DONE.** `reference-broker.sh capabilities` reports a closed `permitted`/`refused` matrix.
- ~~Parse caller action bytes once, canonicalize once, and retain a broker-owned snapshot through launch.~~ **DONE.** `reference-broker-snapshot.py`'s `create_snapshot`/`launch_snapshot`.
- ~~Bind executable bytes and every interpreter involved...~~ **DONE**, for the launch forms currently admitted (native-elf, dash-inline-c).
- ~~Add `launch-pending`, `launch-confirmed`, `launch-denied`, and `launch-ambiguous` events to the existing ledger and recovery path.~~ **DONE (2026-09-05), completing the one real SCOPE-2 gap.** New `dispatch-launch-state` contract type registered in `measured-budget-runtime-contracts.json` (no `graph`/`graphEdges` entry needed — that DAG is optional per-contract and only used for the registry's own internal consistency check, not enforced at runtime). `measured-budget-runtime.py` gained `launch_pending` (refuses a nonce mismatch or a second pending record for the same permit), `launch_confirm`/`launch_deny` (both refuse when no pending record exists, or when a terminal record already does), and `launch_reconcile` (a recovery pass: promotes every permit whose latest launch-state record is still `launch-pending` to `launch-ambiguous`; idempotent — a permit already at a terminal state, ambiguous included, is left alone). `reference-broker.sh` now records `launch-pending` immediately before `permit-consume` and a terminal record (`launch-confirmed` on a successful launch, `launch-denied` if launch preparation fails after consumption) immediately after.
- ~~Preserve one-use permit semantics...~~ **DONE**, landed with the above. `launch_reconcile` never touches the underlying `budget-reservation` — proven directly (`reservation.state == "reserved"` after an ambiguous promotion) rather than assumed. No automatic retry channel was added; `permit_issue`'s pre-existing "one permit per decision" rule (unconditional on outcome) already makes a second attempt on the same decision impossible, proven directly against an ambiguous permit rather than assumed.
- ~~Add adversarial tests that mutate action files, executable paths, shebang interpreters, and explicit interpreters after validation...~~ **DONE, and already landed before this write-up was corrected.** `measured-budget-runtime-v2-selftest.py`'s `TypedGraphTests` carries this coverage; an earlier version of this document incorrectly claimed it did not exist — corrected in the Status line above. The launch-state work adds 10 new pure-runtime-level tests (no subprocess, no Linux dependency — all 10 run and pass on macOS) plus 2 broker-level subprocess tests (Linux/dash-dependent like their siblings; the happy-path one confirmed the exact `["launch-pending","launch-confirmed"]` sequence a real dispatch produces, the other confirmed a pre-snapshot refusal produces no launch-state record at all rather than an incorrectly-assumed `launch-denied`). Both new guards (replay-prevention, reconcile-idempotency) mutation-verified.
...
### SCOPE-4 — Canonical Final-Edge Gateway And Mandatory Permit Composition — **DONE**

New `bubbles/scripts/mutable-dispatch-gateway.sh`. Design decision made before writing any code, driven by Authorization Invariant #5's own wording ("identities SELECTED BY THE BROKER"): the authorization must be minted against the SAME snapshot bytes that are later launched, so the gateway creates ONE snapshot itself (via `reference-broker-snapshot.py`, the same tool the broker uses), mints and verifies the `mutable-dispatch-authorization` against it, and only then hands that immutable directory to the broker — which was extended with a new `--existing-snapshot-dir` flag (additive, optional; every SCOPE-2 caller and selftest that omits it is unaffected and still gets its own full snapshot hardening) so it reuses that exact snapshot instead of re-deriving one from the original mutable path a second time. Re-deriving would have reopened precisely the TOCTOU window SCOPE-2's broker-owned snapshotting exists to close.

- ~~Add one final repository mutable-dispatch gateway over the hardened broker.~~ **DONE.** `mutable-dispatch-gateway.sh dispatch`.
- ~~Require all authorization invariants before process creation and return a closed refusal before any child launch on failure.~~ **DONE.** Order: resolve adapter → create snapshot → fetch the permit record (new `measured-budget-runtime.py` `permit_get`, read-only) → mint → verify → only then invoke the broker. The first failure at any step refuses (exit 1) with nothing launched; proven directly (case G4: a failed repository-authority check leaves zero `permit-consumption` records in the store, not merely a non-zero exit code).
- ~~Require a current IMP-055 one-use permit for every mutable repository command vector. No orchestrator, adapter, or command class receives an implicit exemption.~~ **PARTIALLY DONE.** The gateway itself always requires and verifies a current permit before dispatching. Making it the ONLY path (so nothing can bypass it by calling the broker directly) is SCOPE-6's job (migrating `phase-coordinator.sh` and the four orchestrators); until that lands, a direct `reference-broker.sh` call still works on its own SCOPE-2 hardening, without this gateway's composed authorization.
- ~~Continue using `dispatchAdmission.adapter`; `none` and absence deny child dispatch, while `reference-broker` selects repository-reference enforcement.~~ **DONE.** Resolved via the existing `dispatch-adapter-resolve.sh` (unmodified); `none` (including the absence-resolves-to-`none` default) and any adapter other than exactly `reference-broker` are refused, each with its own distinguishable message — proven as two separate cases (G3, G6) precisely because a single generic check would have made the two indistinguishable by outcome alone, which mutation-testing caught during development (see verification notes).
- ~~Keep route composition and read-only analysis outside the mutable path...~~ **Unaffected, correctly.** This gateway only has one entry point (`dispatch`); it does not touch routing or read-only analysis at all, so this invariant is preserved by scope rather than by an added check.

**Verification.** `mutable-dispatch-gateway-selftest.sh`: 6 cases, driving a REAL MBE store (a full identity→budget→epoch→intent→negotiation→quote→reservation→facts→decision→permit chain, issued through the actual runtime, not stubbed), a real repository packet/control file, and a real frozen goal contract with a real pre-dispatch G134 receipt. 5 of 6 pass on this macOS sandbox; the sixth (G1, the true happy path through an actual process launch) requires `reference-broker-snapshot.py`'s Linux-only precondition, exactly like every SCOPE-2 broker-launch test — confirmed to fail for that reason specifically (the exact "platform is unsupported" message), not a different error, and confirmed the flow correctly stops at snapshot creation, before authorization is even attempted. Two guards mutation-verified (the reference-broker-only allowlist; the none-specific refusal message, which needed its own assertion because the allowlist check alone made the two paths behaviorally indistinguishable — a real, caught-in-development instance of exactly the kind of redundant-coverage question this whole IMP-058 effort spent so much time on elsewhere). `reference-broker.sh`'s full existing SCOPE-2 suite (44 tests) re-run after the `--existing-snapshot-dir` addition: identical pass/fail shape to before the change, zero regressions.

### SCOPE-5 — Post-Execution Result-Envelope Audit Contract — **DONE**

**Terminology check before writing anything:** the framework already has a widely-used "RESULT-ENVELOPE" concept (`completion-governance.md`'s `outcome: completed_owned` / `observations: []` agent-completion schema, referenced by dozens of unrelated guard scripts). That is NOT what this scope means. IMP-056's own EV-17 finding is specifically about `reference-broker.sh`'s own output object, `reference-dispatch-result` — a coincidental name collision this write-up keeps distinct rather than silently conflating.

- ~~Add conditional audit fields for mutable dispatch outcomes without presenting them as pre-execution authority.~~ **DONE.** `reference-dispatch-result` gained `mutableDispatchAudit`, a new field carrying `permitId`, `authorizationAuthenticator`, `launchStateId`, `launchState` — present ONLY when the dispatch was reached through the SCOPE-4 gateway (an `authorization.json` exists in the shared snapshot directory); `null` for a direct broker call, exactly as before this scope.
- ~~Populate fields only from durable broker and ledger records after execution or refusal.~~ **DONE.** Every value is read back from records the ledger already accepted (the just-written `launch-confirm` record; the authorization file the gateway already verified) at the very last line of the script, after settlement. Nothing is computed ahead of the event it describes.
- ~~Keep read-only, advisory, route-only, and blocked-before-dispatch envelopes compatible.~~ **DONE, by construction.** `mutableDispatchAudit` is additive to `reference-dispatch-result`; no schema-registry entry exists for that contract type (confirmed by search) for anything to break, and a REFUSED dispatch (repository authority, boundary, receipt, or authorization failure) never reaches this line at all — it exits before any envelope is printed.
- ~~Add schema and semantic tests that reject claims inconsistent with recorded authorization, permit, launch state, or settlement while proving envelope data cannot open the gateway.~~ **DONE.** Two new tests in `measured-budget-runtime-v2-selftest.py` (Linux-dependent like their SCOPE-2/4 siblings, consistent with the same platform gate): one drives a gateway-shaped dispatch (a hand-built `authorization.json` in an externally-created snapshot directory, since a full mint/verify round trip is already SCOPE-3's own coverage) and asserts every audit field matches the real permit and launch-state record; the other drives a plain direct dispatch and asserts `mutableDispatchAudit` is `null`, never a fabricated or defaulted object. "Cannot open the gateway" is verified, not asserted: a repo-wide search confirms `reference-dispatch-result` has exactly one producer (`reference-broker.sh`) and zero consumers anywhere else in the tree — nothing reads it back as an input to any decision, so a fabricated envelope can misrepresent history to whoever reads the report, never re-enter authorization.
...
- [x] The broker launches only a capability-proven immutable action, executable, and interpreter snapshot and never rereads caller action bytes. **DONE (SCOPE-2).** `reference-broker-snapshot.py`; proven by `test_reference_broker_never_rereads_replaced_action_path` and siblings.
- [x] Unsupported platform, executable, interpreter, or launch-strategy combinations fail closed before permit consumption. **DONE (SCOPE-2).** Validated inside `create_snapshot`, which runs before `permit-consume` and now before `launch-pending` too.
...
- [x] Every mutable repository command vector requires the current IMP-055 one-use permit at the final edge. **PARTIALLY DONE (SCOPE-4).** True for every caller that goes through `mutable-dispatch-gateway.sh`, which always requires and verifies a current permit before dispatching. Making the gateway the ONLY reachable path is SCOPE-6.
- [x] A durable `launch-pending` precedes permit consumption, and every recoverable occurrence reaches one closed terminal launch state. **DONE (SCOPE-2, 2026-09-05).** Proven by `test_launch_pending_then_confirm_is_the_terminal_state`/`..._then_deny_...` (runtime-level) and `test_reference_broker_records_launch_pending_then_confirmed` (broker-level, Linux).
- [x] Unprovable post-consumption state becomes `launch-ambiguous`, holds the full reservation, and cannot automatically retry. **DONE (SCOPE-2, 2026-09-05).** Proven by `test_launch_reconcile_promotes_an_unresolved_pending_to_ambiguous` and `test_launch_ambiguous_holds_the_reservation_and_forbids_a_second_permit`, both mutation-verified at the guard level.
- [x] Result-envelope fields are post-execution audit evidence and cannot authorize process creation. **DONE (SCOPE-5).** `mutableDispatchAudit` on `reference-dispatch-result`; verified to have zero consumers anywhere in the tree.
...
| SCOPE-2 | ~~Reference broker, measured-budget runtime/finalizer, capability probe, focused selftests~~ **DONE** — `reference-broker.sh`, `measured-budget-runtime.py`, `measured-budget-runtime-contracts.json`, `bubbles/schemas/dispatch-admission.schema.json` (regenerated), `measured-budget-runtime-v2-selftest.py` | Harden before gateway work |
...
| SCOPE-4 | ~~Canonical final-edge gateway and integration selftests~~ **DONE** — `mutable-dispatch-gateway.sh` + `-selftest.sh`, `reference-broker.sh` (`--existing-snapshot-dir`, additive), `measured-budget-runtime.py` (`permit_get`, read-only) | Compose all invariants and current permit |
| SCOPE-5 | ~~Result-envelope schema, producers, consumers, semantic tests~~ **DONE** — `reference-broker.sh` (`mutableDispatchAudit`), `measured-budget-runtime-v2-selftest.py` | Audit-only fields |
