...
- Drive `known_downstream_failures` in `v5.3-selftest.sh` to empty. Six at review, five after the first pass, ONE now. Two of the original descriptions were WRONG, and so was one of the corrections; all three are restated here from reproductions rather than from reading.
