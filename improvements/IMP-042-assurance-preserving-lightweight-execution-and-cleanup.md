...
- Make generated, optional, and historical classes explicit alongside managed and source-only. The classes are currently implicit in hardcoded arrays in `generate-release-manifest.sh` (`demoted_source_only_scripts`, plus the installer/eval/regression/bundle-budget appends around lines 124-184).
...
- BLOCKED on a named coupling, not on effort: excluding maintainer-only history from the downstream payload requires making `docs/governance-index.md` payload-aware FIRST. `governance-index-lint.sh` lines 95 and 136 skip a path that does not exist (`[[ -f "$repo_root/$rel" ]] || return 0`), so it enumerates docs ON DISK and requires each to be indexed. The index is itself a managed, shipped doc. Demoting a doc therefore either leaves a dead relative link in the shipped index downstream, or, if the row is removed, makes the still-present source doc an un-indexed orphan and turns the lint red in source. One shipped file cannot satisfy both directions.
