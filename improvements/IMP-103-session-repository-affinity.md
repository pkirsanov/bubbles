...
- [x] The capability entry is exactly `partial`/`unreleased`, carries the required owner/docs/evidence, and its consumer list equals the mechanically derived union. Evidence: independent inspection of the `durable-work-repository-boundary` ledger entry at frozen HEAD `46a4692` — `state: partial`, `releaseIntroduced: unreleased`, `ownerSurface: bubbles/scripts/repository-binding.sh`, 6 `docsRefs`, 10 `evidenceRefs`; the consumer list equals the mechanically derived union, proven by `capability-ledger-selftest.sh` 100 passed/0 failed (clean-ledger contract PASS plus per-consumer `CAP-RB-CONSUMER-<id>` rejections).
- [x] **Test T5A.1:** clean and every capability/state/release/evidence/consumer adversarial case produce the required pass/fail result with current-session raw evidence. Evidence: `bash bubbles/scripts/capability-ledger-selftest.sh` → 100 passed, 0 failed (exit 0) at HEAD `46a4692`; clean ledger satisfies the contract and `CAP-RB-MISSING`, `CAP-RB-STATE`, `CAP-RB-RELEASE`, every `CAP-RB-EVIDENCE-<id>`, and every `CAP-RB-CONSUMER-<id>` fixture is rejected.
- [x] **Test T5A.2:** fixture-derived freshness counts pass and stale generated summaries fail without touching canonical generated files. Evidence: `bash bubbles/scripts/capability-freshness-selftest.sh` → 4 passed, 0 failed (exit 0); fixture-derived counts (22/2/0) pass and both the stale generated capability-guide and the stale issue-status block are detected via temporary fixtures, leaving canonical generated files untouched.
- [x] **Test T5A.3:** exact competitive cross-surface counts pass and arbitrary/mismatched numeric summaries fail. Evidence: `bash bubbles/scripts/competitive-docs-selftest.sh` → 14 passed, 0 failed (exit 0), F-1 hermetic temporary fixture; exact ledger counts (22 shipped/2 partial/0 proposed) agree across README/generated/matrix while the arbitrary summary a loose `[0-9]+` regex would accept is rejected.
- [x] **Test T5A.4:** all three selftests have final LF bytes, use no raw `mapfile`/`readarray`, and pass direct Bash syntax, ShellCheck, shfmt, and macOS portability checks. Evidence: `bash -n` OK on all 3; final byte `0a` on all 3; no raw `mapfile`/`readarray` (the only match is a comment at L87 stating "no mapfile"); `macos-portability-guard.sh` PASS (13/13 classes); enforced `shellcheck -S warning` exit 0 on the 3 files and whole-surface `shellcheck-lint.sh` exit 0; `shfmt -d -i2 -ci -bn` clean (0 diff; non-enforced — adjudicated non-blocking).
- [x] Paired red/green evidence uses identical named fixture mutations and proves the prior validator's false green before the correction. Evidence: false-green baseline in a detached worktree at pre-S5A `63b4f99` — all 3 pre-correction validators green (exit 0) while `capability-ledger-selftest.sh` carries 0 CAP-RB/durable-work enforcement and the capability is absent from the ledger; true-red at HEAD `46a4692` — 16 CAP-RB enforcement points (flip-state, drop-consumer, flip-release, exact-vs-arbitrary), each mutation rejected in the T5A.1 100/0 run; throwaway worktree removed.
- [x] Changed-path evidence contains exactly the ledger plus three selftests and no generated, installer, release, runtime, IMP-102, or governance-index path. Evidence: `git diff --numstat 63b4f99 46a4692` = exactly the 4 S5A files (`bubbles/capability-ledger.yaml` 92/0; `bubbles/scripts/capability-freshness-selftest.sh` 26/3; `bubbles/scripts/capability-ledger-selftest.sh` 358/16; `bubbles/scripts/competitive-docs-selftest.sh` 129/8); zero generated/installer/release/runtime/IMP-102/governance-index bleed.
- [x] Rollback rehearsal restores the four-file baseline and leaves every canonical generated/release file byte-identical. Evidence: commit-level `git diff --numstat 63b4f99 46a4692 -- docs/generated docs/its-not-rocket-appliances.html bubbles/installer/installer.yaml VERSION CHANGELOG.md README.md` = empty (0 lines); an isolated 4-file revert in a temporary worktree touches exactly the 4 S5A files and nothing else, and the worktree was removed. The dirty working-tree generated files are out of S5A scope (owned by S5B).
- [x] `bubbles.audit` independently certifies the committed S5A inventory, adversarial coverage, portability, and exact changed-path boundary before S5B begins. Evidence: this audit pass, at frozen HEAD `46a4692`, independently re-ran T5A.1–T5A.4 (100/0, 4/0, 14/0, all exit 0), inspected the ledger entry, reproduced the paired red/green (`63b4f99` false-green vs `46a4692` true-red), confirmed the changed path is exactly the 4 S5A files, and rehearsed commit-level rollback — S5A inventory, adversarial coverage, portability, and boundary certified before S5B.
- [x] Build Quality Gate: the four-file corrective is warning-free, fixture-only, deterministic, and makes no generated freshness, full validation, release, or push claim. Evidence: warning-free at enforced gates (`shellcheck -S warning` exit 0, `macos-portability-guard.sh` 13/13 PASS, `bash -n` clean); fixture-only (all 3 selftests hermetic against temporary fixtures, canonical generated files untouched); deterministic; and no generated-freshness/full-validation/release/push claim is made — S5B remains blocked.
