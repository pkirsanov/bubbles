...
**Status:** IMPLEMENTED (preflight primitive + installer marker stamping, reuse-first, 2026-07-17) — `repo-binding-preflight.sh` fails loud (exit 1) when an agent's source-repo slug does not match the repository being edited, and passes for `--canonical-source` framework work (MR1/SCOPE-1). `install.sh` now STAMPS the repo-relative `targetRepoSlug` marker into `.github/bubbles/.install-source.json` (SCOPE-5) using the SAME slug derivation as the per-repo MCP server id, flipping the preflight from advisory-until-configured to ENFORCING on every fresh install/upgrade (no per-machine absolute path is committed). Repo-qualified agent labels (MR2/SCOPE-2), provenance-bearing handoff/continuation envelopes (MR3/SCOPE-3), attribution separation (SCOPE-4), and multi-root fixtures (SCOPE-6) remain the wiring follow-up.
