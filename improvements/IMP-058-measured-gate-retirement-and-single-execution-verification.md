...
### SCOPE-1 — Bind the retirement reader to the gate registry (REG-22)

- Change `model-tier-advisory.sh:41` to resolve the gate registry the way `gate-retirement.sh:90` already does: `GATES="${BUBBLES_GATES_FILE:-$REPO_ROOT/bubbles/registry/gates.yaml}"`, and read gates from it. The mode/phase operations continue to read `workflows.yaml`; only the `retirement` operation is re-pointed. Two registries, two variables, no shared default.
- Add a zero-result assertion to `model-tier-advisory-selftest.sh`: `retirement` against the real registry MUST report a non-zero `modelCompensation gates:` count. The defect survived because an empty parse renders as a clean pass, which is the failure mode the assertion names.
- Extend `gate-retirement.sh lint` to fail when the registry declares model-compensation gates and the advisory reports zero, so the two tools cannot silently disagree about the same file again.
- This scope changes no gate, no threshold, and no enforcement. It makes an existing report able to return its own subject.
...
- `bash bubbles/scripts/model-tier-advisory.sh retirement --tier opus-class` reports 25 model-compensation gates with no environment override, and `model-tier-advisory-selftest.sh` fails if that count is zero.
- `gate-retirement.sh lint` fails when the gate registry and the advisory disagree on the model-compensation gate count.
