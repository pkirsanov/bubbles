...
**Status:** IN PROGRESS — SCOPE-1 and SCOPE-2 landed. SCOPE-1: `model-tier-advisory.sh retirement` now binds to `bubbles/registry/gates.yaml` (was silently reading `workflows.yaml`, always reported zero), mutation-verified against the original defect, `gate-retirement.sh lint` cross-checks the two readers' counts. SCOPE-2: added the `preventionEvidence` criterion form; `retirement` now partitions modelCompensation gates into EARNING (6: G027, G040, G041, G061, G068, G084 — prevented at least once), CANDIDATE (4: G085, G090, G092, G128 — fired ≥ 348 declared runs, never prevented), UNMEASURED (15 remaining), reading live from `gate-hit-log.sh --json`; the exact partition matches this proposal's own predicted classification. Two mutations (weakened `>=` boundary, weakened earning threshold) both caught by new assertions. `model-tier-advisory-selftest.sh` 30/30 passing (was 21, +9), `gate-retirement-selftest.sh` 46/46 passing (registry addition didn't disturb lint), shellcheck clean. SCOPE-3 through SCOPE-7 remain PROPOSED.
