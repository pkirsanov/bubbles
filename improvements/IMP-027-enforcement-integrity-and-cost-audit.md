...
The uniform fix for the root cause. Four sub-changes, each independently landable:

- **2a — Bind enforcement in the gate registry.** Add an `enforcedBy` field to each entry in `bubbles/registry/gates.yaml` with a constrained vocabulary: `script:<path>`, `guard-check:<N>`, `ci:<workflow>`, `mode-required`, or `behavioral:<agent>`. Regenerate `docs/generated/gate-coverage-map.md` **from that field** instead of literal gate-ID grepping, eliminating both the false negatives (G018, G035, G078) and the false-positive framing that counts `requiredGates` membership as mechanical coverage. Add a lint that fails when a gate declares `script:`/`guard-check:` that does not resolve. Because the registry is spliced byte-identically into `bubbles/workflows.yaml` by `generate-gates-block.sh`, this change propagates automatically — re-run the generator and `gates-registry-selftest.sh` proves parity.
